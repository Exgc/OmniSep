        ...
        for batch in pbar:
            ...
            with torch.no_grad():
                ...
                if args.is_feature:
                    ...
                    if mode == "image":
                        img_emb.append(F.normalize(batch['frames_feat'][0].mean(1).to(device)))
                    elif mode == "text":
                        img_emb.append(F.normalize(batch['text_feat'][0].to(device)))
                    elif mode == "audio":
                        img_emb.append(F.normalize(batch['audio_feat'][0].to(device)))
